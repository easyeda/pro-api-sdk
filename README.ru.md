...
Это расширение интегрирует помощник по проектированию схем с двумя движками ИИ (GPT-5.1 + Gemini 2.5 Pro), чтобы помочь новичкам легко проектировать электронные схемы.

### Основные возможности

- **Конструктор схем ИИ**: Автоматическое создание схем из описаний на естественном языке
- **Двойной движок ИИ**:
  - GPT-5.1: Глубокое понимание требований пользователей и создание подробных спецификаций проектирования
  - Gemini 2.5 Pro: Визуальная проверка и предложения по оптимизации
- **Дружественный для новичков**: Предоставляет подробные объяснения и рекомендации по безопасности
- **Интеграция с JLCPCB**: Автоматически использует компоненты из каталога JLCPCB

### Как использовать

1. Выберите **"API SDK" > "Настройки ИИ..."** в меню
2. Введите свой ключ API OpenAI (для GPT-5.1)
3. Введите свой ключ API Google Gemini (для Gemini 2.5)
4. Нажмите **"API SDK" > "Конструктор схем ИИ"** для начала использования

### Необходимые ключи API

- **Ключ API OpenAI**: Для модели GPT-5.1 (требуется доступ к GPT-5.1-preview)
- **Ключ API Google Gemini**: Для модели Gemini 2.5 Pro

Подробную информацию об архитектуре интеграции ИИ см. в [AI_INTEGRATION_ARCHITECTURE.md](./docs/AI_INTEGRATION_ARCHITECTURE.md)
